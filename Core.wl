...
RenamableIndexNameQ::usage = "RenamableIndexNameQ[a]";
...
AbsIndexNameQ::usage = "";
DisplayName::usage = "";
DisplayNameOf::usage = "DisplayNameOf[symbol]";
FindTermIndicesList::usage = "FindTermIndicesList[expr]";
...
IndexBoxForm::usage = "IndexBoxForm[index]";
IndexName::usage = "";
...
GroupIndexListWithSlotTypes::usage = "GroupIndexListWithSlotTypes[{i1, slot1}, ...]";
...
NoIndicesQ::usage = "NoIndicesQ[expr]";
NonTensorTermQ::usage = "NonTensorTermQ[expr]";
...
ETensorIndices::usage = "ETensorIndices[inds, nullPos, lengths]";
...
ITensorRowReduce::usage = "ITensorRowReduce[tensor]";
...
IndexToBox[a_Symbol] := SymbolName@a;
IndexToBox[a_] := MakeBoxes@a;
IndexToBox[Verbatim[Pattern][symbol_Symbol, Blank[]]] := SymbolName@symbol <> "_";
SetAttributes[IndexToBox, HoldAll];

IndexBoxForm[a_] := MapAt[IndexToBox, SeparateIndexName[a], 1];
SyntaxInformation@IndexBoxForm = {"ArgumentsPattern" -> {_}};
...
UInd[] := UInd@Null;
UInd[type_] := TempIndex[$TempIndexNumber++];
UInd[type_, n_] := Table[UInd@Null, n];
SyntaxInformation@UInd = {"ArgumentsPattern" -> {_., _., _.}};
...
RenamableIndexNameQ[a_] := RenamingGroupOfIndexName@a =!= Null;
SyntaxInformation@RenamableIndexNameQ = {"ArgumentsPattern" -> {_}};
...
IndexNamedOrder[a_, b_] := LexicographicOrder[SeparateIndexName@a, SeparateIndexName@b];
SyntaxInformation@IndexNamedOrder = {"ArgumentsPattern" -> {_, _}};
...
FindIndicesSlots[Inactive[fn_][args___]] := FindIndicesSlots[fn@args];
...
ExpandScalarIndexScope[expr_Plus] := ExpandScalarIndexScope /@ expr;
ExpandScalarIndexScope[expr_Times] := ExpandScalarIndexScope /@ expr;
ExpandScalarIndexScope[expr_List] := ExpandScalarIndexScope /@ expr;
ExpandScalarIndexScope[Power[expr_, n_?IntegerQ]] := Times @@ Table[ExpandScalarIndexScope@expr, n] /; n > 0;
ExpandScalarIndexScope[ETensor[expr_, inds_]] := ETensor[ExpandScalarIndexScope@expr, inds];
ExpandScalarIndexScope[HoldPattern@IndexScope@expr_] := ExpandScalarIndexScope@ReplaceDummiesToUnique[expr];
...
SymmetryOfExpression[TDagger@expr] ^:= SymmetryOfExpression@expr;
TDagger /: SumPassThroughQ[TDagger@expr_, {1, pos__}] := SumPassThroughQ[expr, {pos}];
TDagger /: ExpressionPassThroughQ[TDagger@expr, tensor_, {1, pos__}] := tensor === TDagger@tensor && ExpressionPassThroughQ[expr, tensor, {pos}];
TDagger /: MakeBoxes[expr : TDagger[expr2_], StandardForm] := With[{
    box = SuperscriptBox[MakeBoxes@expr2, "\[Dagger]"]
}, InterpretationBox[box, expr]];
SyntaxInformation@TDagger = {"ArgumentsPattern" -> {_}};
...
SyntaxInformation@NoIndicesQ = {"ArgumentsPattern" -> {_}};
...
IndexToGridRow[{a_, IndexNameSlot}] := {a, ""};
IndexToGridRow[{a_, DI@IndexNameSlot}] := {"", a};
...
TensorGridBox[t_, inds_List, opt : OptionsPattern[]] := With[{
    ...
    With[{ind = inds2[[1]], indName = inds2[[1, 1]]}, Switch[ind[[2]],
        IndexNameSlot, SuperscriptBox[t, indName],
        DI@IndexNameSlot, SubscriptBox[t, indName]
    ]],
...
DisplayNameOf[name_] := MakeBoxes@name;
SetAttributes[DisplayNameOf, HoldAll];
SyntaxInformation@DisplayNameOf = {"ArgumentsPattern" -> {_}};

FindTermIndicesList[expr_] := FindIndicesSlotsAndNames[expr][[All, 2, 1]];
...
ConverTFLabelSlotFormatingRule[{DI@TFLabelSlot, sym_}] := Hold@{{MakeBoxes@sym, DI@IndexNameSlot}};
ConverTFLabelSlotFormatingRule[{TFLabelSlot[left_, right_], sym_}] := Hold@{{RowBox@{left, MakeBoxes@sym, right}, IndexNameSlot}};
ConverTFLabelSlotFormatingRule[{DI@TFLabelSlot[left_, right_], sym_}] := Hold@{{RowBox@{left, MakeBoxes@sym, right}, DI@IndexNameSlot}};
...
NonIndexSlotQ[DI@expr_] := NonIndexSlotQ@expr;
...
TryReplaceIndexNames[expr_, reps_] := TryReplaceIndexNames[expr, reps, True &];
TryReplaceIndexNames[expr_, reps_, indPredicate_] := With[{
    reps2 = Select[reps, AbsIndexNameQ@#[[1]] &]
}, If[Length@reps2 > 0,
    With[{
        indPoses = Select[
            Select[{#2, Lookup[FindAllIndicesNames@expr, #1, Null]} & @@@ reps2, #[[2]] =!= Null &],
            AllTrue[#[[2, All, 2]], indPredicate] &
        ]
...
SyntaxInformation@TryReplaceIndexNames = {"ArgumentsPattern" -> {_, _, _.}};
...
DefSimpleMetric[sym_, slotType_, symSign_, opt : OptionsPattern[]] := (
    ...
    Switch[OptionValue@ContractionRuleMode,
        ...
        sym /: _?TimesQ[l___, sym[DI@b_, a_?NonDIQ], r___] := With[{
            newExpr = TryReplaceIndexNames[Times[l, r], {a -> b, b -> a}, # === slotType &]
        }, newExpr /; newExpr =!= $Failed];
...
GroupIndexListWithSlotTypes[inds_] := If[Length@inds > 0 && Head@inds[[1]] =!= Rule,
    MapIndexed[#2[[1]] -> #1 &, inds],
    inds
] (* {pos -> {ind, slot}, ...} *) //
    GroupBy[Extract[{2, 2}] -> (#[[1]] -> #[[2, 1]] &)] //
    Map@GroupIndexList //
    (* <|slot -> {{type, ...} -> {name -> {pos, ...}, ...}}|> *)
    KeyValueMap[{slot, data} |-> {slot, #1} -> #2 & @@@ data] //
    Apply@Join;
SyntaxInformation@GroupIndexListWithSlotTypes = {"ArgumentsPattern" -> {_}};

IndexOrder[DI@a_, DI@b_] := Order[a, b];
IndexOrder[_?NonDIQ, _DI] := 1;
IndexOrder[_DI, _?NonDIQ] := -1;
IndexOrder[a_?NonDIQ, b_?NonDIQ] := Order[a, b];
SyntaxInformation@IndexOrder = {"ArgumentsPattern" -> {_, _}};

AddSymmetryToOneIndex[{slot_, pat : {a_, DI@a_}}, indToPos : (indName_ -> {pos1_, pos2_}), pairSym_] := {
    slot,
    pat,
    With[{s = pairSym[indName, pos1, pos2]}, If[s === 0, {}, {s}]],
    RenamingGroupOfIndexName@indName
} -> indToPos;
AddSymmetryToOneIndex[{slot_, pat : {DI@a_, a_}}, indToPos : (indName_ -> {pos1_, pos2_}), pairSym_] := {
    slot,
    pat,
    With[{s = pairSym[indName, pos2, pos1]}, If[s === 0, {}, {s}]],
    RenamingGroupOfIndexName@indName
} -> indToPos;
AddSymmetryToOneIndex[{slot_, pat_}, indToPos : (indName_ -> _), _] := {
    slot,
    pat,
...
AddSymmetryToOneIndexGroup[pairSym_][slotAndPat_ -> indsToPos_] := AddSymmetryToOneIndex[slotAndPat, #, pairSym] & /@ indsToPos;
...
SymmetryGroupOfSymmetricIndexGroup[i_, {_, pat_, symList_, renamingGroup_} -> inds_] := With[{
    blockLen = Length@pat,
...
FindAndDropFrees[list_List, Automatic] := FindAndDropFrees[list, Length@#1[[2]] === 1 &];
...
ITensorReduceOneTermSorted[expr_, frees_, opt : OptionsPattern[]] := With[{
    ...
    actualIndsAndSlot = {Extract[expr, #1], #2} & @@@ indPos
}, With[{
    groupedInds = FindAndDropFrees[GroupIndexListWithSlotTypes[actualIndsAndSlot], frees]
}, With[{
    dummySymList = AddSymmetryToGroupedIndexList[groupedInds[[2]], ExpressionPairSymProvider[expr, indPos, symDummyPairSelector]],
    freeInds = CollectGroupedIndices[MapAt[Extract[2], 1] /@ groupedInds[[1]]]
}, With[{
    indsSym = Join[
        ShiftPermutation[
    ...
    groupedDummies = MapAt[Extract[2], 1] /@ dummySymList
}, With[{
    canonIndsAndPos = {freeInds, CollectGroupedIndices[groupedDummies]}
}, With[{
    ...
    xToolsDebugPrint[ITensorReduceOneTermSorted, "grouped indices: ", dummySymList];
...
TrySplitIndexScopeFactors[factors_, frees0_] := With[{
    inds = Select[Keys@FindAllIndicesNames@#, RenamableIndexNameQ] & /@ factors
}, With[{
...
IndexScope[expr_] := expr /; NoneTrue[Keys@FindAllIndicesNames@expr, RenamableIndexNameQ];
...
ReplaceDummiesToUnique[expr_, frees_List] := With[{
    allInds = Values@KeyDrop[KeySelect[FindAllIndicesNames@expr, RenamableIndexNameQ], frees]
...
MakeIndexRule$UnwrapDoubleHoldPattern[Verbatim[HoldPattern]@expr_HoldPattern] := expr;
MakeIndexRule$UnwrapDoubleHoldPattern[expr_HoldPattern] := expr;
MakeIndexRule$ConstructRule[lhs_, Hold@withArgs_, Hold@rhs_] := If[Hold@withArgs === Hold@{},
    lhs :> rhs
,
    lhs :> With[withArgs, rhs]
];
MakeIndexRule[expr_, frees_] := MakeIndexRule[expr, frees, {}];
MakeIndexRule[expr_List, args__] := MakeIndexRule[#, args] & /@ expr;
MakeIndexRule[lhs_ -> rhs_, args__] := MakeIndexRule[lhs :> rhs, args];
MakeIndexRule[lhs_ :> rhs_, frees_, others_] := With[{
    lhsInds = FindAllIndicesNames@lhs,
    rhsInds = FindAllIndicesNames@rhs
}, With[{
    rhsDummies = Complement[Keys@rhsInds, Join[frees, others]]
}, With[{
    ...
    MakeIndexRule$UnwrapDoubleHoldPattern@ReplacePart[HoldPattern@lhs, Join @@ (Thread[(Prepend[1] /@ lhsInds[#][[All, 3]]) -> Pattern[Evaluate@freeRenames@#, Blank[]]] & /@ frees)],
    JoinHold2[Hold@{# = UInd[]} & /@ Values@dummyRenames],
    ReplacePart[Hold@rhs, Join @@ Join[
        KeyValueMap[{old, new} |-> Prepend[#3, 1] -> (#1 /. IndexNameSlot -> new) & @@@ rhsInds@old, dummyRenames],
        KeyValueMap[{old, new} |-> Thread[(Prepend[1] /@ rhsInds[old][[All, 3]]) -> new], freeRenames]
    ]]
]]]]];
SyntaxInformation@MakeIndexRule = {"ArgumentsPattern" -> {_, _, _.}};
...
MakeIndexFunction[expr_, frees_] := MakeIndexFunction[expr, frees, {}];
MakeIndexFunction[expr_, frees_, otherInds_] := With[{
    inds = FindAllIndicesNames@expr
}, With[{
    freeRenames = Association@MapIndexed[#1 -> Slot @@ #2 &, frees],
    dummyRenames = With[{d = Complement[Keys@inds, Join[frees, otherInds]]}, Association@Thread[d -> GetInternalSymbols@Length@d]]
}, MakeIndexFunction$ConstructFunction[
    ReplacePart[Hold@expr, Join @@ Join[
        KeyValueMap[{old, new} |-> Prepend[#3, 1] -> (#1 /. IndexNameSlot -> new) & @@@ Lookup[inds, old, {}], dummyRenames],
        KeyValueMap[{old, new} |-> Thread[(Prepend[1] /@ Lookup[inds, old, {}][[All, 3]]) -> new], freeRenames]
    ]]
,
    JoinHold2[Hold@{# = UInd[]} & /@ Values@dummyRenames]
]]];
SyntaxInformation@MakeIndexFunction = {"ArgumentsPattern" -> {_, _, _.}};
...
ETensorProduct[prod_, ETensor[expr1_, frees1_], ETensor[expr2_, frees2_]] := With[{
    indPos1 = KeySelect[FindAllIndicesNames@expr1, RenamableIndexNameQ],
    indPos2 = KeySelect[FindAllIndicesNames@expr2, RenamableIndexNameQ]
}, With[{
...
(* special case: numerical multiplication *)
(* needed since identical terms are automatically combined in Plus, given (...) * NITensor[...] and cannot be handled by CombineSummedNITensor *)
NITensor /: n_?NumericQ * NITensor[tensor_, inds_] = NITensor[n * tensor, inds];
...
NITensorReduce$CombineSummedNITensor[head_[NITensor[first_, firstInds_], rest__]] := With[{
...
ITensorRowReduce$OneRow[arr_, row_]
ITensorRowReduce[]

TermToList[expr_] := If[Head@expr === Times, List @@ expr, {expr}];

ITensorInverse::failed = "Unable to invert tensor `1`.";
ITensorInverse$FindStructureOne[expr_, a_, b_] := With[{
    indsByFactor = GroupBy[If[Head@expr === Times, List @@ expr, {expr}], With[{
        inds = Keys@FindAllIndicesNames@#
    }, Which[
        Length@inds === 2, 1,
        inds === {a}, 2,
        inds === {b}, 3,
    ...
    metricTerm = Lookup[indsByFactor, 1, {}],
    vectorATerm = Lookup[indsByFactor, 2, {}],
    vectorBTerm = ReplaceIndexNames[Lookup[indsByFactor, 3, {}], b -> a],
    factor = Times @@ Lookup[indsByFactor, 4, {}]
}, Which[
    Length@metricTerm > 0, 2 -> {factor, Times @@ Join[metricTerm, vectorATerm, vectorBTerm]},
    Length@vectorTerm === 2, 1 -> {factor, vectorATerm},
    True, Throw@$Failed
]]];
ITensorInverse$FindStructure[expr_, a_, b_] := With[{
    terms = GroupBy[ITensorInverse$FindStructureOne[#, a, b] & /@ If[Head@expr === Plus, List @@ expr, {expr}], First -> Extract[2]]
}, With[{
    metricTerms = Lookup[terms, 2, {}],
    vectorTerms = Lookup[terms, 1, {}]
}, If[Length@metricTerms <= 1 && Length@vectorTerms <= 1, {If[Length@metricTerms === 1, metricTerms[[1]], 0], If[Length@vectorTerms === 1, vectorTerms[[1]], 0]}, Throw@$Failed]]];
ITensorInverse[ETensor[expr_, {a_, b_}]] := With[{
    struct = ITensorInverse$FindStructure[expr, a, b]
}, With[{
    metricTerm = struct[[1]],
    vectorTerm = struct[[2]]
}, ETensor[Which[
    metricTerm =!= 0 && vectorTerm === 0, ReplaceIndexNames[metricTerm[[2]], {a -> DI@a, b -> DI@b}] / metricTerm[[1]],
    metricTerm =!= 0 && vectorTerm =!= 0, With[{
        metricInv = ReplaceIndexNames[metricTerm[[2]], {a -> DI@a, b -> DI@b}],
        k2 = ReplaceIndexNames[vectorTerms[[2]], {a -> DI@a}]
    }, metricInv / metricTerm[[1]] ]
], {a, b}]]];
